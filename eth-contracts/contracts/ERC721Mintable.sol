...
contract Ownable {
    ...
    constructor() internal {
        emit OwnershipTransfered(address(0x0), msg.sender);
        _owner = msg.sender;
    }

    //  3) create an 'onlyOwner' modifier that throws if called by any account other than the owner.
    modifier onlyOwner() {
        require( _owner == msg.sender, "called by any account other than the owner");
        _;
    }

    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event OwnershipTransfered(address fromOwner, address toOwnerd);
...
contract Pausable is Ownable {
    ...
    modifier onlyWhenNotPaused() {
        require(!_paused, "currently it is paused");
        _;
    }modifier onlyWhenPaused() {
        require(_paused, "currently it is not paused");
        _;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require( to != ownerOf(tokenId), "given address must not be the owner of tokenid" );

        //  require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require( msg.sender == ownerOf(tokenId) || isApprovedForAll(ownerOf(tokenId), msg.sender), "sender should be the owner");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require( !_exists(tokenId) && to != address(0x0) , "given tokenId already exists or given address is invalid");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(to != address(0x0), "valid address");
